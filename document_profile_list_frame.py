...
class DocumentProfileListFrame(ctk.CTkFrame):
    def __init__(self, master, **kwargs):
        ...
        ctk.CTkLabel(self, text="Perfis de Documento Cadastrados", font=ctk.CTkFont(size=18, weight="bold")).grid(row=0, column=0, padx=20, pady=(20, 10), sticky="w")

        # Add Profile Button
        self.add_button = ctk.CTkButton(self, text="Adicionar Novo Perfil de Documento", command=self._add_profile)
    ...
    def _add_profile(self):
        if hasattr(self.master, 'select_frame_by_name'):
            self.master.select_frame_by_name("document_create")
    ...
    def refresh_data(self):
        ...
        self.profiles = data_manager.load_profiles(DocumentProfile)

        if not self.profiles:
            ctk.CTkLabel(self.list_frame, text="Nenhum perfil de documento cadastrado.").grid(row=0, column=0, padx=20, pady=20)
            return
        ...
        for i, profile in enumerate(self.profiles):
            ...
            action_frame.grid(row=row, column=3, padx=10, pady=5, sticky="w")
    ...
    def _edit_profile(self, profile: DocumentProfile):
        # Navigate to the DocumentProfileFrame and load the profile for editing
        if hasattr(self.master, 'select_frame_by_name'):
            self.master.select_frame_by_name("document_edit", profile)
        else:
            messagebox.showerror("Erro", "Funcionalidade de edição não disponível.")

    def _delete_profile(self, profile: DocumentProfile):
