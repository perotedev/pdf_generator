...
class SpreadsheetProfileFrame(ctk.CTkFrame):
    def __init__(self, master, **kwargs):
        ...
        self.select_file_button = ctk.CTkButton(self.top_frame, text="Selecionar Planilha (Excel)", command=self._select_file)
        ...
        # Save Button
        self.save_button = ctk.CTkButton(self, text="Salvar Perfil", command=self._save_profile)
    ...
    def _save_profile(self, is_editing=False):
        ...
        if not profile_name:
            messagebox.showerror("Erro", "O nome do perfil não pode estar vazio.")
            return
        if not self.column_mappings:
            messagebox.showerror("Erro", "Nenhuma coluna mapeada para salvar.")
            return
        ...
        if not is_editing:
            ...
            if any(p.name == profile_name for p in existing_profiles):
                messagebox.showerror("Erro", f"Já existe um perfil de planilha com o nome '{profile_name}'.")
                return
        ...
        messagebox.showinfo("Sucesso", f"Perfil de planilha '{profile_name}' salvo com sucesso!")
